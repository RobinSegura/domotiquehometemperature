...
import serveur.mobile.metier.IMetier;
import serveur.mobile.metier.TemperatureException;
...
public class TemperatureController {
    ...
    /**
     * L'annotation @Autowired demande à Spring d'injecter dans l'attribut metier de la classe,
     * un composant de type IMetier.
     * La classe Metier du package serveur.mobile.metier implémente cette interface IMetier ; elle est donc
     * capable de fournir une méthode pour traiter un service REST
     * Ceci est indiqué par l'annotation @Service de la classe Metier qui fera de cette
    ...
    public TemperatureReponse getTemperature( @PathVariable("capteur") String nomCapteur) {
         ...
         *      "capteur" est une variable dont la valeur est transmise dans la requête
         *      l'annotation @PathVariable("capteur") associe cette variable au paramètre formel
         *      de la méthode 'nomCapteur'
         ...
         * RQ : les valeurs des variables du service sont transportées par HTTP, donc de type String
         * les paramètres de la méthode pourraient être d'un type primitif Java : int, double...
         * cette valeur serait alors convertie par Spring, avec aventuellement levée d'une
         * exception de décodage
         *
         * La réponse envoyée au client du service est un objet TemperatureReponse qui a 3
         * attributs :
         ...
         * sera sérialisé au format JSON, transporté en HTTP, récupéré dans le client, puis
         * désérialisé, ce qui rend les attributs de l'objet TemperatureReponse disponibles.
         */
